...
const Reviewpage = async ({params}: { params: Promise<{ blogid: number; review:  number }> }) => {
     const { blogid, review } = await params;
     if (  blogid < 0 || blogid > 100) {
        notFound();
     }
    return (
    <div>
        <div className="bg-indigo-900 text-white flex justify-center items-center min-h-screen m-4">
            ...
            <h1> the review id = <span style={{color:review<10 ? "red" : review>10? "teal":"green" }}>{review}</span></h1>
    </div>
    </div>
